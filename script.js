    ...
    fetch('https://api.jikan.moe/v3/search/anime?q=' + search_data)
        ...
        .then(data => {
            for (let i = 0; i < data.results.length; i++) {
                ...
                html += '<div class="card-header"><a class="text-movie text-decoration-none" href="home-detail.html?id=' + obj.mal_id + '">' + obj.title + '</a></div>';
                ...
                html += '<a href="favorite.html?id=' + obj.mal_id + '" class="btn btn-primary">add to <i class="fas fa-heart"></i></a>';
                html += '</div>';
                html += '</div>';
                html += '</div>';
                html += '</div>';
